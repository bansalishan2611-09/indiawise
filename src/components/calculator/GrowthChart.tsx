...
export default function GrowthChart({ data }: { data: GrowthRow[] }) {
  ...
  return (
    <div className="mt-8 print:mt-4 print:break-inside-avoid">
      ...
      <div className="bg-white border border-border rounded-2xl p-6 shadow-sm h-[400px] print:hidden">
        <ResponsiveContainer width="100%" height="100%">
          {renderChart()}
        </ResponsiveContainer>
      </div>

      {/* Print Version (Fixed Width to prevent clipping) */}
      <div className="hidden print:block bg-white border border-border rounded-2xl p-6 shadow-sm mx-auto" style={{ width: '700px', height: '400px' }}>
        {renderChart(650, 350)}
      </div>
    </div>
  );
